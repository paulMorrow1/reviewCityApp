...
const Review = ({
  ...
  return (
    <div className="border-2 border-black p-6 mx-20 my-6 items-center bg-white rounded-sm hover:shadow-[rgba(0,0,15,0.15)_0px_0px_10px_3px] transition-shadow ease-in-out">
      <p className="font-bold text-lg cursor-default">
        {review.user.firstName} {review.user.lastName}
      </p>
      <p className="text-sm cursor-default">{review.createdAt.toString()}</p>
      <p className="mt-1 cursor-default">{review.rating}</p>
      <p className="mt-2 mb-6 cursor-default">{review.content}</p>
      ...
      {userId === reviewUserId ? (
        <>
          <AddEditReview businessId={businessId} review={review} />
